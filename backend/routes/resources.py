...
def get_validated_store(table_name, permission_action):
    ...
    permission_error = require_table_permission(table_name, permission_action)
    if permission_error:
...
def list_entities(table_name):
    store, error_response = get_validated_store(table_name, "read")
    if error_response:
...
def create_entity(table_name):
    store, error_response = get_validated_store(table_name, "create")
    if error_response:
        return error_response
...
def update_entity(table_name, entity_id):
    store, error_response = get_validated_store(table_name, "update")
    if error_response:
        return error_response
...
def delete_entity(table_name, entity_id):
    store, error_response = get_validated_store(table_name, "delete")
    if error_response:
        return error_response
