...
export default function Lieferanten() {
    ...
    const gefilterteDaten = useMemo(() => crud.data
        ...
        .map(item => ({
            ...item,
            vorgaenge: {
                label: "Bestellungen und Rechnungen öffnen",
    ...
    return <>
        ...
        <section className="module-panel">
            <div className="personalakte-toolbar">
                <div className="personalakte-links">
                    <Link className="button-link" to="/partnerhistorie?typ=lieferant">Partnerhistorie öffnen</Link>
                </div>
            </div>
        </section>
        <DataTable
        ...
        <Dialog
            open={crud.open}
            title={crud.editMode ? "Lieferant bearbeiten" : "Neuer Lieferant"}
            onClose={crud.handleClose}
            footer={<SaveButton onSave={crud.speichern} onSuccess={crud.handleClose}>Speichern</SaveButton>}
        >
            ...
            <div className="form-row">{crud.error && <p className="form-error">{crud.error}</p>}</div>
        </Dialog>
    </>;
}
