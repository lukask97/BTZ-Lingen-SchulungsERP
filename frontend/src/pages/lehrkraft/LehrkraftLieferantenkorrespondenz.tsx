...
export default function LehrkraftLieferantenkorrespondenz() {
    ...
    const dashboardTabs = [
        ...
        { key: "bestaetigt", label: "Bestaetigt", value: getPurchaseOrdersByStatus(bestellungen, "bestaetigt").length },
    ...
    const angebotSpeichern = () => {
        ...
        if (!bestellung || !lieferant || preis <= 0 || lieferzeitTage <= 0) {
            setOfferError("Bitte Lieferant, Angebotspreis und Lieferzeit ausfuellen.");
            return;
        }
    ...
    return <>
        ...
        <p>Hier begleitet die Lehrkraft den vereinfachten Einkaufsprozess. Die Schuelerfirma erfasst Artikelnummern und Mengen, die Lehrkraft erstellt darauf aufbauend ein Angebot, bestaetigt die Anfrage und markiert die Bestellung anschliessend als versendet.</p>
        ...
        {(activeTab === "anfragen" || activeTab === "bestaetigt" || activeTab === "versendet") && <DataTable
            ...
            rowActions={[
                ...
                { name: "approve", label: "Bestaetigen", permission: PERMISSIONS.GF_BEARBEITEN, onClick: bestaetigen, variant: "success", isVisible: row => canConfirmPurchaseOrder(row), isDisabled: row => !row.lehrkraftAngebotAm },
        ...
        <Dialog open={offerOpen} title="Lehrkraftangebot erfassen" onClose={() => {
            setOfferError("");
            setOfferOpen(false);
        }}>
            <div>
