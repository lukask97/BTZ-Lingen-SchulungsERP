...
export default function Suche() {
    ...
    const targets = useMemo(() => {
        ...
        return [
            ...safeGetAll(() => kundenService.getAll()).map(item => ({ id: `kunde-${item.id}`, nummer: item.kundenNr, bezeichnung: item.firma, bereich: "Kunden", ziel: `/kundenfocus=${item.id}` })),
            ...safeGetAll(() => lieferantenService.getAll()).map(item => ({ id: `lieferant-${item.id}`, nummer: item.lieferantenNr, bezeichnung: item.firma, bereich: "Lieferanten", ziel: `/lieferantenfocus=${item.id}` })),
            ...safeGetAll(() => artikelService.getAll()).map(item => ({ id: `artikel-${item.id}`, nummer: item.artikelNr, bezeichnung: item.name, bereich: "Artikel", ziel: `/artikelfocus=${item.id}` })),
            ...angebote.map(item => {
                ...
                return {
                    ...
                    bereich: "Angebote",
                    ziel: auftrag ? `/vertriebsdokumente/auftrag/${auftrag.id}` : `/angebotefocus=${item.id}`
                };
            }),
            ...
            ...safeGetAll(() => bestellungenService.getAll()).map(item => ({ id: `bestellung-${item.id}`, nummer: item.bestellNr, bezeichnung: item.lieferant || "Bestellung", bereich: "Bestellungen", ziel: `/bestellungenfocus=${item.id}` })),
